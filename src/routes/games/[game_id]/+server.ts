...
export const DELETE: RequestHandler = async (requestEvent) => {
    ...
    const result: boolean = await deleteGame(gameId);
...
export const PUT: RequestHandler = async (requestEvent) => {
    ...
    logger.debug(
        `playTurn PUT gameId: ${gameId}, userId: ${userId}, roundSeq: ${roundSeq}, weaponPlayed: ${weaponPlayed}, responseTimeMillis: ${responseTimeMillis}`
    );
